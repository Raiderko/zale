        ...
        let answers = [
          ...
          let ips = [
            ...
            "51.071.124.129.12.0",
              ...
              if (!taggedUser) {
                return message.channel.send(":x: Tag the person who you want to hack!");
              }
              ...
              .setTitle(`Hacked ${taggedUser.username}`)
              .setFields({
                  name: `Info`,
                  value: `Email:\nsmallPP${taggedUser.username}${answer}\nPassword: ${passwrd}`
              }, {
                  name: `Last DM`,
                  value: `${dm}`,
              }, {
              ...
              const emoji = client.emojis.cache.get(`869625185219801098`)
              let mm = await message.channel.send(`Hacking  ${taggedUser}${emoji}`);
              message.channel
                .send(`Status:  0%: Finding discord login${emoji} (2fa bypass)`)
                .then((msg) => {
                  wait(1000);
                  msg.edit(
                    `Found:\nEmail:\n<@${taggedUser}>@blue.com\nPassword: ${passwrd}`
                  ...
                  msg.edit(`Last DM: ${dm}`);
                  wait(1000);
                  msg.edit(`Finding most common word${emoji}`);
                  wait(1000);
                  msg.edit(`const mostCommon = ${comonWord}`);
                  wait(1000);
                  msg.edit(`Injecting trojan virus into users pc${emoji}`);
                  wait(1000);
                  msg.edit("Virus successfully stolen and emojies stolen");
                  wait(1000);
                  msg.edit(`Setting up Epic Store account${emoji}`);
                  wait(700);
                  msg.edit(`Hacking Epic Store account${emoji}`);
                  wait(1000);
                  msg.edit(`Finding IP adress${emoji}`);
                  wait(1000);
                  msg.edit(`IP adress: ${ip}`);
                  wait(1000);
                  msg.edit(`Selling data to Government${emoji}`);
                  wait(900);
                  msg.edit(`Reporting account to discord for breaking tos${emoji}`);
                  wait(400);
                  msg.edit(`Hacking medical records${emoji}`);
                  wait(600);
                  msg.edit(`IMPORTANT health update: ${taggedUser}'s brain is not found`);
                  wait(500);
                  msg.edit(`finished hacking ${taggedUser}`);
                  wait(1000)
                  msg.edit({embeds: [embed]}).then(mm.delete())
                });
    }
}