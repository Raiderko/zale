...
function stations(client, prefix, message) {
  ...
  try {
    let amount = 0;
    const stationsembed = new MessageEmbed().setColor("PURPLE"
    ...
    const stationsembed2 = new MessageEmbed().setColor("PURPLE"
    ...
    const stationsembed3 = new MessageEmbed().setColor("PURPLE"
...
async function swap_pages(client, message, description, TITLE) {
  ...
  if (embeds.length === 0) return message.channel.send({
    embeds: [new MessageEmbed()
      ...
      .setColor(es.wrongcolor).setThumbnail(`https://cdn.discordapp.com/emojis/918875075564953600.png`
