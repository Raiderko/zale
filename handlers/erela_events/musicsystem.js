...
module.exports = client => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isButton()) {
      ...
      switch (interaction.customId) {
        case "Skip": {
          ...
          }).then(async (msg) => setTimeout(() => msg.delete(), 6000))
          //edit the message so that it's right!
      ...
      case "Lyrics": {
        ...
        const fetch = require("node-fetch")
              ...
              try {
              let song = player.queue.current.title 
              if(!song) return message.channel.send({embeds: [new MessageEmbed()
              ...
              const json = await fetch(`https://some-random-api.ml/lyrics?title=${encodeURIComponent(song)}`).then(r => r.json())
              if (json.error) return interaction.reply({embeds: [new MessageEmbed()
                  .setTitle(`:x: Cannot send lyrics!`)
    ...
    if (interaction.isSelectMenu()) {
      let link = "https://open.spotify.com/playlist/2Y21t4hWd9EtU2cKeQVWLb?si=3055bb8d8d324dee";
      if (interaction.values[0]) {
        //ncs | no copyrighted music
        if (interaction.values[0].toLowerCase().startsWith("n")) link = "https://open.spotify.com/playlist/7sZbq8QGyMnhKPcLJvCUFD";
        //pop
        if (interaction.values[0].toLowerCase().startsWith("p")) link = "https://open.spotify.com/playlist/2Y21t4hWd9EtU2cKeQVWLb?si=3055bb8d8d324dee";
        //default
...
function generateQueueEmbed(client, guildId, leave) {
  ...
  if (!leave && player && player.queue && player.queue.current) {
    embeds[1].setImage(`https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg`)
      ...
      .setAuthor(`${player.queue.current.title}`, "https://i.gifer.com/1kME.gif", player.queue.current.uri)
