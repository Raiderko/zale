...
module.exports = async (client, message) => {
  ...
  db.findOne({ Guild: message.guild.id, Member: message.author.id }, async(err, data) => {
    ...
    if(data) {
      ...
      .setDescription(`${message.author}, I have removed your AFK!`)
      .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
      .setTimestamp()
      .setColor("PURPLE"
)
      message.channel.send({ embeds: [afk]}).then(async (msg) => setTimeout(() => msg.delete(), 10000))
      message.member.setNickname(`${data.Nick}`)
    } else return;
  })
  ...
  if(message.mentions.members.first()) {
    db.findOne({ Guild: message.guild.id, Member: message.mentions.members.first().id }, async(err, data) => {
      ...
      if(data) {
        ...
        message.reply({ embeds: [afk], content: `${message.author}`}).then(async (msg) => setTimeout(() => msg.delete(), 8000))
      } else return;
    })
  }
        ...
        const cmdd = args.shift()?.toLowerCase();
        const cmd = cmdd.toLowerCase()
        ...
        if (command) {
          ...
          if (command.category == "Prem mem") {
            client.logger(command.name)
            let premiumdata = keys.findOne({ user: message.author.id }, async(err, data) => {
              if(!data) {
                not_allowed = true;
                message.reply({
                  embeds: [
            ...
            if(command.parameters.type == "music"){
              ...
              if (!channel) {
                ...
                return message.reply({embeds: [new MessageEmbed()
                  .setColor(es.wrongcolor)
                  .setFooter(es.footertext, es.footericon)
                  .setTitle(`Please Join a VC!`)]}).catch(()=>{})
              }
              ...
              if(command.parameters.activeplayer){
                if (!player){
                  ...
                  return message.reply({embeds: [new MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setFooter(es.footertext, es.footericon)
                    .setTitle(`There are no songs playing!`)]}).catch(()=>{})
                }
                if (!mechannel){
